...
for i, alpha in enumerate([0.3, 0.5, 0.7]):
    eigs = np.power(np.arange(p) + 1, -alpha-1.0)
    eigs /= np.sum(eigs)
    x = np.arange(epochs)+1
    losses = train_single_epoch(lr=lr, alpha=alpha+1.0, p=p, p_task=p, cnt=10000, s=None, epochs=epochs, init=init, y_scale=s)
    ...
    plt.plot(x, losses, linestyle='dotted', color=f'C{i}', linewidth=2)
    plt.plot(x, theo(x-1, eigs, lr, s, init), linestyle='solid', color=f'C{i}', label=r'$\alpha=$'+f'{alpha}')
...
#plt.legend(legend1)
plt.savefig(f'plot/simul_time_learning_curve', dpi=300, bbox_inches='tight')
plt.savefig(f'plot/simul_time_learning_curve.pdf', format='pdf', dpi=300, bbox_inches='tight')
