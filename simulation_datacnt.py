...
from train_package.train2 import train_loop
from toy_model import train_multi_epoch
...
for i, alpha in enumerate([0.3]):
    ...
    eigs /= np.sum(eigs)
    ds = np.array([10,20,50,100,200,1000,2000])
    losses = []
    for d in ds:
        loss = train_multi_epoch(lr=lr, alpha=alpha+1.0, p=p, p_task=p_task, cnt=10000, s=None, epochs=epochs, init=init, y_scale=s, opt='adam')
