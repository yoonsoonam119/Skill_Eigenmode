...
if __name__ == '__main__':
    ...
    d.add(ListSampler(np.arange(1,50,2)), 'width')
    d.add(ListSampler(np.arange(20)), 'repeats')
    zero_mean_str = 'zero_' if args.zero_mean else ''

    for d_args in d:
        param_dict = {'bits': args.bits, 'skill_cnt': 1, 'batch_mul': 5, 'lr':0.05,'alpha':d_args['alpha'], 'init':d_args['init'],
