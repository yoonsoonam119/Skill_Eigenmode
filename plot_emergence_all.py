...
def plot_ax(ax, type_name, xs, theo, param_dict, theparam, sel_idxs=None, alp=True):
    ...
    if type_name == 'time':
        ...
        idx = 2
        ax.set_xscale('log')
    ax.set_title(latex_transform(f'{name} Emergence',idx,alp=alp), y=-0.4, fontdict={'size':12})
    box = ax.get_position()
...
def plot_all(alphas, p = 1, alp=True):
...
if __name__ == '__main__':
    ...
    plot_all([0.3 ,0.6, 0.9], p = 10000, alp=False)
    exit()
