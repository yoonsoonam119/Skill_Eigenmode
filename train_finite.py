...
def run(bits, data_cnt, skill_cnt=5, batch_mul=200, lr=0.001,alpha=2.0, skill_bit_cnt=3, init=0.1, y_scale=3, opt='sgd', act='relu'):
    ...
    init_vals = check_init_vals(skill_cnt, load_creator, model, optimizer)
    for i in range(skill_cnt):
        _, _, _, skill_loss = train_loop(model, skill_tr_loaders[i], skill_te_loaders[i], optimizer, report=False,
                                         epochs=0, criterion=nn.MSELoss(), m=nn.Identity())
...
if __name__ == '__main__':
    ...
    d.add(ListSampler([1000,2000,5000,10000,20000,50000,100000,200000,500000,1000000]), 'data_cnt')
    ...
    for d_args in d:
        ...
        np.save(f'data/finite_skill_losses_{dict_str}', np.stack(skill_losses_arr))
        np.save(f'data/finite_init_vals_{dict_str}', np.stack(init_vals_arr))
        np.save(f'data/finite_te_loss_{dict_str}', np.stack(te_losses_arr))
