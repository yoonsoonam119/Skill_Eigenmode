...
def run(bits, skill_cnt=5, batch_mul=200, lr=0.001,alpha=2.0, skill_bit_cnt=3, init=0.1, y_scale=3, opt='sgd', act='relu'):
    load_creator = Loader(bits=bits,skill_cnt=skill_cnt,skill_bit_cnt=skill_bit_cnt, alpha=alpha, y_scale=y_scale, large=True)
    ...
    for epo in range(10000):
        ...
        print('main', epo)
        train_acc, test_acc, train_loss, test_loss = train_loop(model, train_loader, test_loader, optimizer, report=False,
                                                                epochs=1, criterion=nn.MSELoss(), m=nn.Identity())
...
if __name__ == '__main__':
    ...
    for d_args in d:
        if args.iter_dict:
            param_dict = {'bits': args.bits, 'skill_cnt': 100,
            ...
            param_dict = {'bits': 32, 'skill_cnt': 100, 'batch_mul': 5, 'lr':0.1,'alpha':1.5, 'init':0.05,
        ...
        for _ in range(try_cnt):
            ...
            test_losses = run(**param_dict)
            test_losses_arr.append(test_losses)
        np.save(f'data/test_losses_{dict_str}', np.stack(test_losses_arr))
        #print(np.load(f'data/test_losses_{dict_str}.npy'))
