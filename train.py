...
if __name__ == '__main__':
    ...
    parser.add_argument("-a", "--act", help="activation", type=str, default='tanh')
    ...
    for d_args in d:
        if args.iter_dict:
            ...
            param_dict = {'bits': 16, 'skill_cnt': 5, 'batch_mul': 5, 'lr':0.05,'alpha':1.5, 'init':0.04,
        ...
        for _ in range(try_cnt):
            ...
            skill_losses, model, init_vals, skill_mask = run(**param_dict)
            print(skill_mask)
            print(init_vals)
        ...
        np.save(f'data/skill_losses_{dict_str}', np.stack(skill_losses_arr))
        np.save(f'data/init_vals_{dict_str}', np.stack(init_vals_arr))
